...
class UserRequestHandlerTest {
    ...
    @BeforeEach
    void setUp() {
        handler = new UserRequestHandler(userService);
    }
    ...
    class GetWithId {
        ...
        void shouldReturnHttpStatus200WhenUserIdExists() {
            ...
            when(userService.findByUserId(any(UserId.class))).thenReturn(user);
            APIGatewayProxyResponseEvent response = handler.execute(requestEvent);

            assertEquals(200, response.getStatusCode());
            verify(userService, times(1)).findByUserId(any(UserId.class));
        }
        ...
        <T extends RuntimeException> void assertErrorResult(APIGatewayProxyRequestEvent requestEvent,
            ...
            if (expectedCallNumber > 0) {
                when(userService.findByUserId(any(UserId.class))).thenThrow(clazz);
            }

            APIGatewayProxyResponseEvent response = handler.execute(requestEvent);
            assertEquals(statusCode, response.getStatusCode());
            verify(userService, times(expectedCallNumber)).findByUserId(any(UserId.class));
        }
    }
    ...
    class Post {
        ...
        void shouldReturnHttpStatus201WhenAllIsOk() {
            ...
            when(userService.handle(any(RegisterNewUserCommand.class))).thenReturn(new UserId("id"));
            APIGatewayProxyResponseEvent response = handler.execute(requestEvent);

            assertEquals(201, response.getStatusCode());
            verify(userService, times(1)).handle(any(RegisterNewUserCommand.class));
        }
        ...
        void shouldReturnHttpStatus400WhenBodyIsInvalid() {
            ...
            verify(userService, times(0)).handle(any(RegisterNewUserCommand.class));
        }
        ...
        void shouldReturnHttpStatus500WhenUnexpectedError() {
            ...
            when(userService.handle(any(RegisterNewUserCommand.class))).thenThrow(new RuntimeException());
            APIGatewayProxyResponseEvent response = handler.execute(requestEvent);
